...
impl super::RealmDatabase
{
    ...
    {
        let res = sqlx::query_as!(DBCharacterAccountData, "SELECT * FROM character_account_data WHERE character_id = ?", character_id)
    ...
    {
        sqlx::query!("INSERT INTO character_account_data (character_id, data_type, time, decompressed_size, data) VALUES (?,?, 0, 0, NULL)", character_id, data_type)
    ...
    {
        sqlx::query!("UPDATE character_account_data SET decompressed_size = ?, data = ?, time = ? WHERE character_id = ? AND data_type = ?", 
