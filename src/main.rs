...
async fn handle_incoming_connection(mut stream: TcpStream, database_pool:std::sync::Arc<sqlx::MySqlPool>) -> Result<()>
{
    ...
    let mut logindata = LoginNumbers::default();
    ...
    loop
    {
        ...
        if read_len > 0
        {
            if buf[0] == 0
            {
                logindata = handle_logon_challenge(&mut stream, &buf, &database_pool).await.unwrap();
            }
            else if buf[0] == 1
            {
                handle_logon_proof(&mut stream, &buf, &logindata).await.unwrap();
            }
            else if buf[0] == 2
            {
...
async fn handle_logon_challenge(stream : &mut TcpStream, buf : &[u8], database_pool:&std::sync::Arc<sqlx::MySqlPool>)
    -> Result<LoginNumbers>
{
...
async fn generate_vs(hashed_password : &String, g : &BigUint, n : &BigUint) -> Result<(BigUint, BigUint)>
{
...
async fn handle_logon_proof(stream : &mut TcpStream, buf: &[u8], logindata : &LoginNumbers) -> Result<()>
{
    println!("logon proof");
